...
import { DaysDropdown } from "./ui/days-dropdown";
...
export function HourlyForecast() {
  ...
  return (
    <div className="w-[384px] h-[693px] bg-neutral-800 rounded-lg p-6 flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-1xl font-medium text-white">Hourly forecast</h2>
        
        <div className="relative">
          <button 
            className="px-4 py-2 bg-neutral-700 text-white text-base rounded-lg hover:bg-[#5a5f7f] transition-colors flex items-center gap-2" 
          ...
          {isDaysOpen && (
            <div className="absolute right-0 mt-2 z-10">
              <DaysDropdown onSelect={handleDaySelect} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {hourlyData.map((hour, index) => (
          ...
          >
            ...
            <span className="text-1xl font-medium text-white w-24">
              {hour.time}
            </span>
            <span className="text-1xl font-medium text-white ml-auto">
              {hour.temp}°
            </span>
          </div>
        ))}
